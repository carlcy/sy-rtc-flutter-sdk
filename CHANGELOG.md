...
- **移除 CDN 旁路推流（直播）能力**：删除 `SyLiveTranscoding` 等旁路 API；不再调用 `/api/rtc/live/*`。
- **产品功能位**：`hasLiveFeature` / `live` → `hasRtcFeature` / `rtc`；`hasVoiceFeature` 保留为兼容别名。

### 新增

- `SyRtcVideoView` + 原生 PlatformView（Android/iOS）绑定本地/远端视频
- Dart `onKicked` / `onServerMuteAudio` 事件透出
- Example 精简为单页进房 + 音视频预览流程

## 3.0.1

- Production zip pack excludes `ios/.symlinks`, `.dart_tool`, `build`, `**/Pods`, `**/.git`.
- Example defaults HTTPS/WSS; `--dart-define=SY_API_BASE=`.
- No API breaks vs 3.0.0.

## 3.0.0

### 重大变更

- **适配 Flutter 3.38+**：升级 Kotlin 至 2.0.21、AGP 至 8.7.3，解决 Kotlin 二进制兼容性问题
- Android SDK 依赖从 `compileOnly` 改为 `api`（传递性依赖），用户无需手动添加 Android SDK 依赖
- Java 目标版本从 11 升至 17

### 兼容性说明

- Flutter 3.27.x 及以下用户请继续使用 `2.1.1`
- Flutter 3.38+ 用户请使用 `3.0.0`
