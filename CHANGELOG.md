...
- Production zip pack excludes `ios/.symlinks`, `.dart_tool`, `build`, `**/Pods`, `**/.git` (smaller, clean `flutter pub get`).
- Example defaults remain HTTPS/WSS on IP (`https://47.105.48.196` / `wss://…/ws/signaling`); single switch via `--dart-define=SY_API_BASE=` / docs CLIENT_TRUST.
